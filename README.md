...
* [`src/cljs/simple_re_frame_app/`](src/cljs/simple_re_frame_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/simple_re_frame_app/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `simple-re-frame-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/simple_re_frame_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns simple-re-frame-app.example
  (:require [simple-re-frame-app.config :as config])
